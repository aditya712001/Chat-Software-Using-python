...
class Client:
    ...
    def gui_loop(self):
        def browseFiles():
            ...
            #     self.sock.send(b'#')
            #     # self.sock.send(message.encode('utf-8'))  
            self.sock.sendall(f"{os.path.basename(filepath)}{SEPARATOR}{self.nickname}{MSG_DELIMITER}".encode('utf-8'))
            f = open(filepath, 'rb')
            data = f.read()
            self.sock.sendall(data)
            self.sock.sendall(ENDTAG.encode('utf-8'))#ascii
            f.close()
        ...
        sv = StringVar()
        sv.trace("w", lambda name, index, mode, sv=sv: self.typing_indicator_callback(sv))
        self.input_area = tkinter.Entry(width=100, textvariable=sv,font="times 20",bg="#CCCCCC")
        self.input_area.pack(side=tkinter.LEFT, padx=10)
    ...
    def write(self):
        ...
        self.sock.send(message.encode('utf-8'))
        self.input_area.delete(0, len(message))
        # self.input_area.delete('1.0', 'end')
        
    def typing(self):
        import time
        self.last_presstime = time.time()
    ...
    def typing_indicator_callback(self,sv):
        content = sv.get()
        print(content)
        if content != '':
            self.typing()
    ...
    def receive(self):
            ...
            while self.running:
                # try:
                    ...
                    if(len(message)>0 and message[0] == '@'): # New User Joined
                    ...
                    #         file.write(filedata)
                        ...
                        # client data storage
                        client_data = self.nickname + '_data'
                        if not os.path.exists(client_data):
                            os.makedirs(client_data)
                        file_byte_info = from_user.split(MSG_DELIMITER)
                        filepath = os.path.join(client_data, "From_" + file_byte_info[0] + '__' + filename)
                        print(file_byte_info[0])
                        f = open(filepath, 'wb')
                        
                        file_bytes = b''
                        if len(file_byte_info) > 1:
                            file_bytes += (file_byte_info[1].encode('utf-8'))
